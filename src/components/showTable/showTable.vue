<template>
	<div class="show">
		<div class="search">
			...
			<input type="text" placeholder="请输入房间号" @keypress.enter="pageIndex=1;getData()" v-model="search" />
			&nbsp;&nbsp;
		...
		<div class="searchdata">
			<table>
				...
				<thead>
					<tr>
						<th>序号</th>
						<th v-for="(i,index) in showParms.thead" :class="i.style==undefined?'':i.style">
							{{i.name}}
						</th>
						<th v-for="(i,index) in showParms.action" :class="i.style==undefined?'':i.style">
							{{i.name}}
						</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(i,index) in showData">
						...
						<td v-for="(j) in showParms.thead">
							{{i[j.key]}}
						</td>
						<td v-for="(j) in showParms.action">
							<button type="button" @click="$emit('showTableClick',{key:j.key,playLoad:i})"
							 :class="i.style==undefined?'':j.style">
							 {{j.name}}
							 </button>
						</td>
